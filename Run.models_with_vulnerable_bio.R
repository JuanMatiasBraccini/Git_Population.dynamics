...
if(First.run=="YES")
{
  ...
  if(BaseCase=="Size-based")
  {
    ...
    #TL-TwT
    fn.plt.rel1(TL.bin.mid,TwT.bin.fem,add="YES",TL.bin.mid,TwT.bin.mal,"pink",4)
    ...
    ADD="NO"
    if(length(which(Tabla.scen$Model_type=="Length-based" & Tabla.scen$Maturity=="knife edge"))>0) ADD="YES"
    fn.plt.rel1(TL.bin.mid,Mat.bin,add=ADD,TL.bin.mid,ifelse(TL.bin.mid>=Maturity.at.size[1],1,0),"grey20","grey70")
    ...
    ADD="NO"
    if(length(which(Tabla.scen$Model_type=="Length-based" & Tabla.scen$M=="at length"))>0) ADD="YES"
    fn.plt.rel1(TL.bin.mid,rep(unique(Mort.F.k),length(TL.bin.mid)),add=ADD,TL.bin.mid,Mort.bin,"grey20","grey70")
    ...
    ADD="NO"
    if(length(which(Tabla.scen$Model_type=="Age-structured" & Tabla.scen$Maturity=="at age"))>0) ADD="YES"
    fn.plt.rel1(age.F,Mat.k,add=ADD,age.F,Mat.age,"grey20","grey70")
    if(ADD=="YES")legend("bottomright",c("knife-edge","at age"),lty=1,lwd=2,col=c("grey20","grey70"),bty='n',cex=1.25)
    ...
    ADD="NO"
    if(length(which(Tabla.scen$Model_type=="Age-structured" & Tabla.scen$Fec.=="at age"))>0) ADD="YES"
    fn.plt.rel1(age.F,FecU*Mat.k,add=ADD,age.F,FecU_age,"grey20","grey70")
    if(ADD=="YES")legend("bottomright",c("knife-edge","at age"),lty=1,lwd=2,col=c("grey20","grey70"),bty='n',cex=1.25)
  ...
  # Shapes=mledist(Rprior,"beta")$estimate
  # plot(density(Rprior))
  # lines(density(rbeta(1000,shape1=Shapes[1],shape2=Shapes[2])),col=2)
  Prior.mean.r=mean(Rprior)
  log_Prior.sd.r=10*sd(Rprior)
  
  fn.fig("Prior_r", 2000, 2000)
   par(las=1,mai=c(1,1.15,.1,.15),mgp=c(3.5,.75,0))
   plot(density(rlnorm(10000, meanlog = log(Prior.mean.r), sdlog = log_Prior.sd.r)),
  ...
  plot(Yr,Cpue.all[,match('Mean',names(Cpue.all))],ylab="",xlab="",
       ylim=c(0,max(c(Cpue.all[,match('Mean',names(Cpue.all))],Cpue.folly[,match("cpue",names(Cpue.folly))]))))
  points(Yr[1:nrow(Cpue.all.monthly)],Cpue.all.monthly$Mean,pch=19)
  lines(Yr,Cpue.folly[,match("cpue",names(Cpue.folly))],lwd=2.5,col=2)
  legend("topright",c("Standardised","Effective"),text.col=1:2,cex=1.5,bty='n')
  
  plot(Yr,Cpue.all[,match('Mean',names(Cpue.all))],ylab="",xlab="",col="transparent",ylim=c(0,max(c(Cpue.zn1$Mean,Cpue.zn2$Mean,Cpue.West$Mean))))
  if(nrow(Cpue.West)>0)
  {
  ...
  for(i in 1:nrow(Scenarios))
  {
    ...
    if(d$Model_type=="Biomass dynamics")
    {
      ...
      La.lista$mu_r=Prior.mean.r 
      La.lista$SD_r=log_Prior.sd.r
  ...
  par(new=T)
  sipiui=rbind(Cpue.all.monthly[,1:2],Cpue.all.daily[,1:2])
  plot(yrs.ktch.cpue,sipiui$Mean, axes=F, xlab=NA, ylab=NA,pch=21,bg=2,cex=2,cex.axis=1.25,ylim=c(0,max(sipiui$Mean)))
  ...
  axis(4,round(seq(min(Cpue.all$Mean),max(Cpue.all$Mean),length.out = 4),2),cex.axis=1.25)
...
if(Run.all.Scenarios=="NO")      #Size structured with movement takes 29 mins
{
  #run .tpl
  ...
  Store.Reports[[i]]=reptoRlist(paste(Spec,".rep",sep=""))  # Report file  
...
if(First.run=="YES")
{
  if(Do.zero.Ktch=="YES")
  {
    fn.check.model=function(MODEL,yrs.projections)
    {
      ...
      NoCatch.future.std=subset(NoCatch.future.std,name%in%c("Total_biom", "Virgin_Total_biom"))
      
      #export figures
      fn.showX=function(Y,YLB,REL)
      {
        y=NoCatch.future[[match(Y,names(NoCatch.future))]]
        if(is.matrix(y)) y=rowSums(y)
        if(REL=="YES")y=y/NoCatch.future.std$value[match("Virgin_Total_biom",NoCatch.future.std$name)]
        x<<-1:length(y)
      ...
      #Total catch
      fn.showX(Y="TC_out",YLB="",REL="NO")
      mtext("Total catch (tonnes)",2,line=3.5,las=3,cex=2.1)
      
      #Total biomass
      fn.showX(Y="Total_biom",YLB="",REL="NO")
      ...
      rm(x)
      mtext("Total biomass",2,line=3.5,las=3,cex=2.1)
...
fn.obs.age.len=function(X,what,what1,cl)
{
  ...
  YLIM=c(0,max(c(OBS,PRED$PRED)))
  plot(X,OBS,ylab="",xlab="",pch=19,cex.axis=1.5,cex=2,ylim=YLIM,col=cl)
...
fn.see.Biom=function(X,what,deplet,Spatial)
{
  OBS=subset(STD,name==what,select=c(value, std.dev))
  if(!is.na(deplet))Dpltn=subset(STD,name==deplet,select=c(value, std.dev))
...
fn.model.outputs=function(SCENARIO)
{
  ...
  if(!Scenarios$CPUE[i]%in%c("N/A","No"))
  {
    ...
    if(Spatial.Str=="Single zone")
    {
      ...
      if(!is.na(match("CPUE_eff",names(MOD))))  fn.cpue.simple.res(what="CPUE_eff",what1="Est_CPUE_eff")else
        fn.cpue.simple.res(what="CPUE_out",what1="Est_CPUE_out")     #ACA
  ...
  if(!is.na(id))
  {
    ...
    fn.see.Biom(X=Yrs,what="Total_biom",deplet="depletion",Spatial=Spatial.Str)
    if(Spatial.Str=="Three zones")
    { 
  ...
  if(!is.na(id))
  {
    ...
    fn.lay.single(c(.8,1.3,.05,.05))
    fn.see.Biom(X=Yrs,what="Fem_spawn_biom",deplet=NA,Spatial=Spatial.Str)
    if(Spatial.Str=="Three zones")
    { 
      mtext("Financial year",1,outer=T,line=.5,cex=2)
  ...
  #Vulnerable biomass
  id=match("Vul_biom",names(MOD))
  if(!is.na(id))
  {
    fn.fig("Biomass_vulnerable",2400,2400) 
    fn.lay.single(c(.8,1.3,.05,.05))
    fn.see.Biom(X=Yrs,what="Vul_biom",deplet=NA,Spatial=Spatial.Str)
    if(Spatial.Str=="Three zones")
    { 
      mtext("Financial year",1,outer=T,line=.5,cex=2)
      mtext("Vulnerable biomass (tonnes)",2,outer=T,line=-.5,las=3,cex=2)
    }else
    {
      mtext("Financial year",1,outer=T,line=-1.5,cex=2)
      mtext("Vulnerable biomass (tonnes)",2,outer=T,line=-2,las=3,cex=2)
    }
    dev.off()
  }
  ...
  if(!is.na(id))
  {
    ...
    if(Scenarios[i,]$Model_type=="Length-based")fn.see.Biom(X=Yrs,what="Annual_rec",deplet=NA,Spatial=Spatial.Str)else
    {
  ...
  if(!is.na(match("siz_comp_F",substr(names(MOD),1,10))))
  {
    ...
    par(mfcol=c(2,1),mai=c(.2,.2,.1,.1),oma=c(3,4,1,1),las=1,mgp=c(1,0.7,0))
  ...
  if(Spatial.Str=="Three zones")     
  {
    ...
    par(mgp=c(2,1,0))
    fn.show.MTM(MATRIZ=Move_after_year,N.int=50,"",'YES')  #ACA
...
fn.pdf.model.outputs=function(SCENARIO)
{
  ...
  if(!is.na(id))
  {
    ...
    mtext("Financial year",1,outer=T,line=-0.5,cex=2)
    mtext("Tonnes",2,outer=T,line=-1.0,las=3,cex=2)
    mtext("Vulnerable biomass",3,outer=T,line=-1.75,cex=1.5)
  }
  ...
  if(!is.na(match("siz_comp_F",substr(names(MOD),1,10))))
  {
    ...
    #Obs vs Pred age-length    
    par(mfcol=c(2,1),mai=c(.2,.2,.3,.1),oma=c(3,4,2,1),las=1,mgp=c(1,0.7,0))
    fn.obs.age.len(MOD$AgE,"L","Lpred","black")
...
if(Run.all.Scenarios=="YES")
{
  ...
  if(Do.cols=="NO")colfunc <- colorRampPalette(c("black", "grey90"))

  
  fn.comp.bios=function(VAR,scaler,VAR1,WHERE,add.lgn)
  {
    ...
    MaxY=1
    fn.plt(miniL[[1]][1:length(Yrs)]/Init[[1]])
    for( i in 2:length(miniL))
    {
      aa=miniL[[i]][1:length(Yrs)]/Init[[i]]
      aa[aa>1]=1
    ...
    if(add.lgn=="YES")
    {
      ...
      legend(WHERE,LGn,bty='n',cex=1.1,lwd=3,col=COLs.SCEN,lty=LTy.SCEN)
    }
  ...
  {
    Zen=Compr.grup[n]
    fn.comp.bios(VAR="Total_biom",scaler=1e0,VAR1=Bo,WHERE="bottomleft",add.lgn="YES")
  ...
  dev.off() 
    ...
    #Vulnerable biomass
  fn.fig("Compare_biomass_vulnerable",2400,2200)
  smart.par(n.plots=length(Compr.grup),MAR=c(2.2,2,1,1),OMA=c(2.5,2.5,.5,.5),MGP=c(1,.6,0))
  Bo=Scenarios$Model_type
  names(Bo)=names(Store.Reports)
  Bo=ifelse(Bo=="Biomass dynamics",NA,
            ifelse(Bo=="Age-structured","Virgin_Vul_biom",
                   ifelse(Bo=="Length-based","Virgin_Vul_biom",
                          NA)))
  for(n in 1:length(Compr.grup))
  {
    Zen=Compr.grup[n]
    fn.comp.bios(VAR="Vul_biom",scaler=1e0,VAR1=Bo,WHERE="bottomleft",add.lgn="YES")
    fn.mTxt();fn.mTxtY();fn.Axs()
    mtext(Title.Compr.grup[n],3,cex=1)
  }
  dev.off() 
  ...
  {
    Zen=Compr.grup[n]
    fn.comp.bios(VAR="Fem_spawn_biom",scaler=1e0,VAR1=Bo,WHERE="bottomleft",add.lgn="YES")
    fn.mTxt();fn.mTxtY();fn.Axs()
    mtext(Title.Compr.grup[n],3,cex=1)
  }
  dev.off() 
  ...
  THeSe=nrow(subset(Scenarios,!CPUE%in%c("No","N/A")))
  fn.fig("CPUEs",2400,2400) 
  par(mfrow=n2mfrow(THeSe+2),mai=c(.3,.4,.1,.1),oma=c(2,2,.5,.5),las=1,mgp=c(2,.85,0))
  for( i in 1:THeSe)
  {
      MOD=Store.Reports[[i]]
      Fze=Par.phases[[i]]
      ...
      legend("topright",paste(Scenarios$Model[i]),bty='n',cex=1.5,inset=c(-.025,-0.05),text.col="black")
   }
...
if(DO.MCMC=="YES")
{
  ...
  #2.2 Stats  
  setPath(paste("2_Outputs/Model_outputs/",ID.base.Model,"/MCMC",sep=""))
  ...
  Plot.Index=function(DAT,FINYEAR,COL,Colr,Colr2,Do.axis,Add.Re.Pnt,do.polygons,btarget)
  {
    ...
    if(Add.Re.Pnt=="YES")
    {
      ...
      text(YR[Now]*.9,btarget*.65,paste("P(>",B.target,")=",Prob.above,sep=""),pos=1,cex=2,col="brown4")
    }
  ...
  Display.plot.index=function(Post,Post.init,AXS,Add.Re.Pnt,Relative,Do.polys)
  {
    ...
    Plot.Index(DAT=Post.rel,FINYEAR=All.yrs,COL,Colr,Colr2,AXS,Add.Re.Pnt,Do.polys,btarget)  
  }
  ...
  density.fun=function(what)
  {
    ...
    plot(D$x,D$y,main='',ylab="",type='l',lwd=2,cex.axis=1.5,yaxt='n')
  ...
  par(mfcol=c(3,1),mai=c(.1,.2,.1,.1),oma=c(4,4,2,.1),las=1,mgp=c(1,.6,0))
  ...
  Display.plot.index(Post=Posteriors[[match("Total_biom",names(Posteriors))]],
                     ...
                     AXS="NO",Add.Re.Pnt="YES",Relative="NO",Do.polys="NO")
  mtext("Total  (tonnes)",2,cex=1.3,las=3,line=3.5)
  
  # vulnerable biomass
  Display.plot.index(Post=Posteriors[[match("Vul_biom",names(Posteriors))]],
                     Post.init=Posteriors[[match("Virgin_Vul_biom",names(Posteriors))]],
                     AXS="NO",Add.Re.Pnt="YES",Relative="NO",Do.polys="NO")
  mtext("Vulnerable  (tonnes)",2,cex=1.3,las=3,line=3.5)
  ...
  Display.plot.index(Post=Posteriors[[match("Fem_spawn_biom",names(Posteriors))]],
              ...
              AXS="YES",Add.Re.Pnt="YES",Relative="NO",Do.polys="NO")
  mtext("Female mature (tonnes)",2,cex=1.3,las=3,line=3.5)
  
  mtext("Biomass",3,line=0,cex=1.5,outer=T)
  mtext("Financial year",1,line=3,cex=1.5)
  ...
  par(mfcol=c(3,1),mai=c(.1,.1,.1,.1),oma=c(4,4,2,.1),las=1,mgp=c(1,.6,0))
  
  # total biomass     
  Display.plot.index(Post=Posteriors[[match("Total_biom",names(Posteriors))]],
                     Post.init=Posteriors[[match("Virgin_Total_biom",names(Posteriors))]],
                     AXS="NO",Add.Re.Pnt="YES",Relative="YES",Do.polys="NO")
  mtext("Total",2,cex=1.5,las=3,line=2.5)
  
  # vulnerable biomass
  Display.plot.index(Post=Posteriors[[match("Vul_biom",names(Posteriors))]],
                     Post.init=Posteriors[[match("Virgin_Vul_biom",names(Posteriors))]],
                     AXS="NO",Add.Re.Pnt="YES",Relative="YES",Do.polys="NO")
  mtext("Vulnerable",2,cex=1.5,las=3,line=2.5)
  
  # Mature biomass
  Display.plot.index(Post=Posteriors[[match("Fem_spawn_biom",names(Posteriors))]],
                     Post.init=Posteriors[[match("Virgin_Spawn_Biom",names(Posteriors))]],
                     AXS="YES",Add.Re.Pnt="YES",Relative="YES",Do.polys="NO")
  mtext("Female mature",2,cex=1.5,las=3,line=2.5)
  ...
  par(mfcol=c(2,1),mai=c(.1,.1,.1,.1),oma=c(3,4,.1,.1),las=1,mgp=c(1,.6,0))
    ...
    #females
  Display.plot.index(Post=Posteriors[[match("Annual_F_F",names(Posteriors))]],
                     Post.init=NA,AXS="NO",Add.Re.Pnt="NO",Relative="NO",Do.polys="NO")
  legend('topright',"Females",bty='n',cex=1.5)

    #males
  Display.plot.index(Post=Posteriors[[match("Annual_F_M",names(Posteriors))]],
                     Post.init=NA,AXS="YES",Add.Re.Pnt="NO",Relative="NO",Do.polys="NO")
  ...
  par(mfcol=c(3,1),mai=c(.1,.1,.2,.1),oma=c(4.5,3.5,.1,.1),las=1,mgp=c(1,.75,0))
  
    # total biomass
  density.fun(what=Posteriors[[match("depletion",names(Posteriors))]])
  legend("topright","Total",cex=2.5,bty='n')
  
    # vulnerable biomass
  density.fun(what=Posteriors[[match("depletion_vul",names(Posteriors))]])
   legend("topright","Vulnerable",cex=2.5,bty='n')
  
    # Mature biomass
   density.fun(what=Posteriors[[match("depletion_spawn",names(Posteriors))]])
  legend("topright","Female mature",cex=2.5,bty='n')
  
  mtext("Density",2,cex=2,line=1,outer=T,las=3)
  mtext(paste(Current,"relative biomass"),1,line=3,cex=2)
  dev.off()
}
