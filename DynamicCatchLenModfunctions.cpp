...
// [[Rcpp::export]]
NumericVector SelAtLength_cpp(const double SelL50, const double SelL95, 
                              const NumericVector midpt, const int nLenCl) {
  // selectivity at length relationship
  NumericVector SelAtLength(nLenCl);
  
  SelAtLength = 1 / (1 + exp(-log(19) * (midpt - SelL50) / (SelL95 - SelL50)));
  // std::cout << "SelAtLength " << SelAtLength << std::endl;
  return SelAtLength;
}
...
// [[Rcpp::export]]
List CalcGillnetSelectivity_cpp(const double theta1, const double theta2, const NumericVector MeshSize_mm,
                                const int nMeshes, const int nLenCl, const NumericVector midpt) {
  ...
  double temp1;
  double temp2;
  NumericVector alpha(nMeshes);
  NumericVector beta(nMeshes);
  NumericVector alpha_beta(nMeshes);
  NumericVector SumSelAtLengthForMesh(nLenCl);  
  NumericVector SelAtLength(nLenCl);
  NumericMatrix SelAtLengthForMesh(nMeshes,nLenCl);
  ...
  for (m=0; m<nMeshes; m++) {
    for (i=0; i<nLenCl; i++) {
      temp1 = midpt(i) / alpha_beta(m);
      temp2 = alpha(m);
      SelAtLengthForMesh(m,i) = pow(temp1,temp2) * exp(alpha(m) - midpt(i) / beta(m));
      
      SumSelAtLengthForMesh(i) = SumSelAtLengthForMesh(i) + SelAtLengthForMesh(m,i);
    }
  }
  SelAtLength = SumSelAtLengthForMesh / max(SumSelAtLengthForMesh);
    
    return List::create(Named("SelAtLengthForMesh") = SelAtLengthForMesh,
                        Named("SelAtLength") = SelAtLength);
}
...
List GetBiologyAndFisheryParams_cpp(const NumericVector Ages, const int nAges, const int nLenCl, const NumericVector midpt, 
                                    ...
                                    const int SelectivityOption, const double theta1, const double theta2, 
                                    const NumericVector MeshSize_mm, const int nMeshes, const double SelL50, const double SelL95) {
  ...
  if (SelectivityOption==1) {
    Selres = CalcGillnetSelectivity_cpp(theta1, theta2, MeshSize_mm, nMeshes, nLenCl, midpt);
    SelAtLength = as<NumericVector>(Selres["SelAtLength"]);
  }
  if (SelectivityOption==2) {
    SelAtLength = SelAtLength_cpp(SelL50, SelL95, midpt, nLenCl); // logistic selectivity    
  }
