...
fn.input.data=function(SP,Yr.assess,Conv.cal.mn.to.fin.mn,Historic.Ktch,Bin.size,What.Efrt)
{
  ...
  Catch_1949=read.csv("Catch and Effort/Heald(1987)_1949_total_catch_by port_allspecies.csv",stringsAsFactors=F) 
  ...
  #1.4 Heald 1987
  #description= All shark catch for southern WA (Geraldton to SA border). Weight in tonnes
  Catch_1952_1975=read.csv("Catch and Effort/Historical_WA_shark_catches_1952_1975.csv",stringsAsFactors=F) 
  names(Catch_1952_1975)=c("year","LIVEWT.c")
  
  #1.5 TDGDLF
  #Total catch data (all fishing methods)
  Data.monthly=read.csv("C:/Matias/Analyses/Catch and effort/Data_outs/Data.monthly.csv",stringsAsFactors=F)
  ...
  #1.6 Northern Shark Fisheries (WANCSF and JANSF)
  Data.monthly.north=read.csv("C:/Matias/Analyses/Catch and effort/Data_outs/Data.monthly.north.csv",stringsAsFactors=F)
  Data.monthly.north=subset(Data.monthly.north,SPECIES%in%Species)
  Data.monthly.north$LIVEWT.c=Data.monthly.north$LIVEWT

  #1.7 Other Fisheries  
    #1.7.1 Other fisheries reporting shark catch in monthly or daily logbooks
  
  
    #1.7.2 Wetline  
  #Western Rock lobster
  #description: catch of dusky and sandbar sharks in droplines set by lobster fishers
  WRL=read.csv("Catch and Effort/WRL/Number-of-vessels.csv",stringsAsFactors=F)
  WRL.Wann=read.csv("Catch and Effort/WRL/Wann_catch.csv",stringsAsFactors=F)
  ...
  WRL=subset(WRL,Finyear%in%Used.yrs)
    ...
    #1.7.3 Commonwealth Southern and Western Tuna and Billfish Fisheries
  ...
  #page 43 Borg & McAuley 2004: 1165 individuals of bronzey in 2001, also catches for previous years... DISCUSS!
    ...
    #1.7.4 Estuaries (CAES, Kim Smith??)
  
    #1.7.5 Pilbara trawl (Corey W.)
  ...
  Rec.fish.catch=rbind(Rec.fish.catch.2011.12,Rec.fish.catch.2013.14)
  Rec.fish.catch$Bioregion=with(Rec.fish.catch,ifelse(Bioregion=="Gasconye","Gascoyne",Bioregion))
  ...
  Overzd.Whler=read.csv("Catch and Effort/TEPS/Comments from SharkDailyTripReturns.csv",stringsAsFactors=F)
    ...
    #1.9.2 Other fisheries
  TEPS_other=read.csv("Catch and Effort/TEPS_catch.csv",stringsAsFactors=F)
  
  # 1.10 SA Marine Scalefish fishery (source Taylor et al 2015)
  ...
  if(SP=="WH")
  {
   ...
   # Ab.indx.TDGDLF.West=read.csv(paste(HNDL,"Whiskery shark.annual.abundance.basecase.monthly.West.csv",sep=""),stringsAsFactors=F)
    ...
    Ab.indx.TDGDLF.all.daily=read.csv(paste(HNDL,"Whiskery shark.annual.abundance.basecase.daily.csv",sep=""),stringsAsFactors=F)
      ...
      #monthly km.gn.hours
    Ab.indx.TDGDLF.all.hours=read.csv(paste(HNDL,"Whiskery shark.annual.abundance.basecase.monthly_hours.csv",sep=""),stringsAsFactors=F)
    
      #daily km.gn.hours
    Ab.indx.TDGDLF.all.daily.hours=read.csv(paste(HNDL,"Whiskery shark.annual.abundance.basecase.daily_hours.csv",sep=""),stringsAsFactors=F)
        
    #Folly cpue
    Ab.folly.TDGDLF.all=read.csv(paste(HNDL,"Whiskery shark.annual.folly.csv",sep=""),stringsAsFactors=F)
  }
  ...
  if(SP=="GM")
  {
    ...
    Ab.indx.TDGDLF.all.daily=read.csv(paste(HNDL,"Gummy shark.annual.abundance.basecase.daily.csv",sep=""),stringsAsFactors=F)
      ...
      #monthly km.gn.hours
    Ab.indx.TDGDLF.all.hours=read.csv(paste(HNDL,"Gummy shark.annual.abundance.basecase.monthly_hours.csv",sep=""),stringsAsFactors=F)
    
      #daily km.gn.hours
    Ab.indx.TDGDLF.all.daily.hours=read.csv(paste(HNDL,"Gummy shark.annual.abundance.basecase.daily_hours.csv",sep=""),stringsAsFactors=F)
    
    #Folly cpue
    Ab.folly.TDGDLF.all=read.csv(paste(HNDL,"Gummy shark.annual.folly.csv",sep=""),stringsAsFactors=F)
  }
  ...
  if(SP=="BW")
  {
    ...
    #Folly cpue
    Ab.folly.TDGDLF.all=read.csv(paste(HNDL,"Dusky shark.annual.folly.csv",sep=""),stringsAsFactors=F)
  }
  ...
  if(SP=="TK")
  {
    ...
    #Folly cpue
    Ab.folly.TDGDLF.all=read.csv(paste(HNDL,"Sandbar shark.annual.folly.csv",sep=""),stringsAsFactors=F)
    
    #Naturaliste survey
    Sanbar.Ab.index.Srvy.FixSt=read.csv("C:/Matias/Analyses/Surveys/Naturaliste_longline/outputs/Index/Sandbar.Srvy.FixSt.csv",stringsAsFactors=F)
    Sanbar.Ab.index.Srvy.AllSt=read.csv("C:/Matias/Analyses/Surveys/Naturaliste_longline/outputs/Index/Sandbar.Srvy.AllSt.csv",stringsAsFactors=F)
  }
 ...
 Rep.Recap$Recaptured=as.character(Rep.Recap$Recaptured)
  ...
  #(in 1000 km gn days)
  if(What.Efrt=="km.gn.days") Eff.zn=read.csv("C:/Matias/Analyses/Catch and effort/Data_outs/Annual.zone.eff.days.csv",stringsAsFactors=F)
  
  #(in 1000 km gn hours)
  if(What.Efrt=="km.gn.hours") Eff.zn=read.csv("C:/Matias/Analyses/Catch and effort/Data_outs/Annual.zone.eff.hours.csv",stringsAsFactors=F)
  ...
  source("C:/Matias/Analyses/SOURCE_SCRIPTS/Population dynamics/Organise input parameters.R")
 ...
 if(SP=="TK")
 {
  ...
  #note: decided not to include the historic catches (see reasons in Assessment report)
    ...
    fn.fig("C:/Matias/Data/Catch and Effort/Pre_1975_total_shark_catch",2400, 2400) 
    par(mfcol=c(1,1),las=1,mgp=c(2.9,.6,0))
    with(subset(Historic.ktch,year<1975),plot(year,LIVEWT.c, ylab="Total shark landings (tonnes)",xlab="Financial year",
  ...
  #note: TDGDLF is method==GN or LL. Others are different methods and the catch is reported in "Data.request".
  ...
  historic.prop.ktch$LIVEWT.c=historic.prop.ktch$LIVEWT.c*Mean.prop.ctch
     ...
     #remove duplicated data for other gears
  TDGDLF.other.gears=subset(Data.monthly,SPECIES%in%Species & !METHOD%in%c("GN","LL"),
                select=c(FINYEAR,LIVEWT.c,SPECIES,zone,MONTH,VESSEL))    
 
  TDGDLF.other.gears$dummy=with(TDGDLF.other.gears,paste(FINYEAR,MONTH,VESSEL,zone))
  Data.request=subset(Data.request,SPECIES%in%Species)
  
  Data.request$FINYEAR=as.character(with(Data.request,
          ifelse(MONTH%in%1:6,paste(YEAR-1,"-",fn.subs(YEAR),sep=""),
          ifelse(MONTH%in%7:12,paste(YEAR,"-",fn.subs(YEAR+1),sep=""),NA))))
  Data.request$LIVEWT.c=Data.request$LIVEWT.c/1000   #in tonnes
  Data.request$LAT=-as.numeric(substr(Data.request$BLOCKX,1,2))
  Data.request$LONG=100+as.numeric(substr(Data.request$BLOCKX,3,4))
  Data.request$zone=as.character(with(Data.request,ifelse(LONG>=116.5 & LAT<=(-26),"Zone2",
  ...
  Data.request$dummy=with(Data.request,paste(FINYEAR,MONTH,VESSEL,zone))
 ...
 Monthly.prop$prop=Monthly.prop$prop/sum(Monthly.prop$prop)
  ...
  if(SP%in%c("BW","TK"))
  {
    fn.check.NSF=function(a)
    {
      ...
      plot(b$LIVEWT.c/1000,type='l',lwd=2)
    }
    ...
    NSF$LIVEWT.c=NSF$LIVEWT.c/1000    #in tonnes
  ...
  Data.request=subset(Data.request,LAT<=(-26))
  other_ktch=aggregate(LIVEWT.c~FINYEAR+MONTH+zone,Data.request,sum)
  
  
  #1.5. Wetline catches of dusky sharks                   
  #rock lobster
  ...
  if(SP=="BW")
  {
    ...
    Annual.Dusky.Ktch.WRL.Wann=sum(Dusky.WRL$LiveWt)/1000 #in tonnes
  ...
  #1.6 SWTBF catches of dusky and sandbar sharks               
  ...
  for(i in 2:ncol(AFMA_catch))AFMA_catch[,i]=AFMA_catch[,i]/1000   #convert to tonnes
  ...
  if(SP%in%c("BW","TK"))
  {
    ...
    if(Option==2)    
    {
      ...
      #Dusky_WTBF=AFMA_catch[,c(1,6)]  
      Dusky_WTBF[,2]=Effort_WTBF[,2]*PCM*cpue_dusky_WTBF*Mean.wt.dusky/1000
      Sandbar_WTBF=Effort_WTBF[,1:2]
      #Sandbar_WTBF=AFMA_catch[,c(1,7)]
      Sandbar_WTBF[,2]=Effort_WTBF[,2]*PCM*cpue_sandbar_WTBF*Mean.wt.sandbar/1000
     }
  ...
  fn.rec=function(DAT)
  {
    DAT$LIVEWT.c=round(DAT$Kept.Number+DAT$Rel.Number*PCM.rec)*AVG.wt/1000   #catch weight in tonnes
  ...
  if(SP=="BW")
  {
    ...
    #Catch in tonnes is number dead X average weight of 3 m sharks + PCM * (number alive + number unknown)
    Mn.wt=bwt*300^awt/1000
 ...
 if(SP=="BW")
 {
  ...
  if(SP=="TK")
  {
    ...
    #monthly
    Ab.indx.TDGDLF.West.daily$zone="West"
    Ab.indx.TDGDLF.daily=Ab.indx.TDGDLF.West.daily
  }
  ...
  if(SP=='TK')
  {
    Sanbar.Ab.index.Srvy.FixSt$FINYEAR=paste(Sanbar.Ab.index.Srvy.FixSt$FINYEAR,"-",fn.subs(Sanbar.Ab.index.Srvy.FixSt$FINYEAR+1),sep="")
    Sanbar.Ab.index.Srvy.AllSt$FINYEAR=paste(Sanbar.Ab.index.Srvy.AllSt$FINYEAR,"-",fn.subs(Sanbar.Ab.index.Srvy.AllSt$FINYEAR+1),sep="")
    Sandbar.abun=Sanbar.Ab.index.Srvy.FixSt
  }
  ...
  #set working directory for outputing data
   ...
   setwd(paste(HandL,SP.LABELS,"/",Yr.assess,"/1_Inputs/Visualise data",sep=''))
 ...
 dev.off()
  ...
  fn.see.Ktch=function(DAT,YRS,NAMES,where,cx.zn,cx.Titl,cx.Man,cx.axs,tck1,tck2,tck3)
  {
    ...
    for(i in 1:length(DAT))
    {
      ...
      a=a[order(a$FINYEAR),]
      if(nchar(round(max(a$LIVEWT.c,na.rm=T)))>5)a$LIVEWT.c=a$LIVEWT.c/1000
      if(!is.logical(a))
      {
 ...
 write.csv(Shots.SF,"Shots.SF.csv",row.names=F)
  ...
  PAR()
  #par(mai=c(.7,2.1,.1,.1))
  visualize.dat(CATCH=catch,SIZE=All.size[1],ABUNDANCE=Ab.folly.TDGDLF.all,C.TAGS=c.Tag,
  ...
  #TDGLDF cpue
    #folly
  write.csv(Ab.folly.TDGDLF.all,"cpue.annual.TDGDLF.folly.csv",row.names=F)
  ...
  #Naturaliste survey     
  if(SP=="TK") write.csv(Sandbar.abun,"cpue.annual.survey.csv",row.names=F)
